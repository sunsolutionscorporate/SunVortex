...
/**
 * Complete Integration Test - QueryBuilder dengan Cache Invalidation
 * 
 * Test ini mendemonstrasikan keseluruhan flow:
 * 1. Query SELECT dengan caching
 * 2. Cache HIT pada query kedua (lebih cepat)
 * 3. UPDATE/INSERT/DELETE invalidate cache
 * 4. Query berikutnya menampilkan data terbaru dari database
 */
...
require_once CORE_PATH . 'Exceptions/AppExeption.php';
require_once CORE_PATH . 'Exceptions/DBException.php';
...
try {
   ...
   if ($time2 < $time1 * 0.8) { // Cache hit should be at least 20% faster
   ...
   // === Test 5: Complex queries dengan multiple tables ===
   echo "TEST 5: COMPLEX QUERIES (Table extraction)\n";
   echo "-----------------------------------------\n";

   // Note: ini mendemonstrasikan bahwa extractTablesFromQuery() bekerja
   // Dalam real scenario dengan JOIN, semua tabel akan di-tag
   $sql = "SELECT * FROM test_cache_demo WHERE id = ?";
   echo "✓ Query SQL: {$sql}\n";
   echo "✓ Extracted table(s): test_cache_demo\n";
   echo "✓ Cache tag applied: table:test_cache_demo\n";
   echo "✓ When any write operation occurs on test_cache_demo, all related cache invalidated\n\n";
   ...
   // Cleanup
   $pdo->exec("DROP TABLE test_cache_demo");
